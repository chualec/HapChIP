  ...
  for rec in vcf.fetch():
    ...
    if check_var(rec) and (GT == "1|0" or GT == "0|1"):
      ...
      for bam_read in bam_in.fetch(var_chr, var_pos-1, var_pos):
        ...
        if check_read(bam_read): #this is to help avoid weird situations where there is no cigar string.
...
def main(options):
  ...
  bam_out_0 = pysam.AlignmentFile(output_name+".hap0.bam", template = bam_in, mode = "wb")
  bam_out_1 = pysam.AlignmentFile(output_name+".hap1.bam", template = bam_in, mode = "wb")
  bam_out_2 = pysam.AlignmentFile(output_name+".hap2.bam", template = bam_in, mode = "wb")
