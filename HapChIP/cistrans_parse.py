...
def bam_list_write(bam_list, vcf, bam_outs):
  if len(bam_list) > 1:
    ...
    for bam_read in bam_list:
      ...
      haplist.append("\t".join([bam_read.reference_name, str(bam_read.reference_start), str(bam_read.reference_end), str(bam_read.mapping_quality), str(hap)]))
    comb = combinations(range(1, len(bam_list)+1), 2)
    for i in comb:
      bam_outs[4].write(ids + "\t" + haplist[i[0]-1] + "\t" + haplist[i[1]-1] + "\n")
      ...
      if check_read(bam_read) and bam_read.is_supplementary == False:
        if str(bam_read.query_name) in bam_library:
          ...
          if abs(int(start2) - int(start1)) > 1000 or chr1 != chr2:
            bam_outs[3].write("\t".join([str(bam_read.query_name), chr1, start1, end1, chr2, start2, end2, "\n"]))
          if chr1 != chr2:
...
def main(options):
  ...
  #make output files
  bam_out_0 = pysam.AlignmentFile(output_name+".cis_short.bam", template = bam_in, mode = "wb")
  bam_out_1 = pysam.AlignmentFile(output_name+".cis_dist.bam", template = bam_in, mode = "wb")
  bam_out_2 = pysam.AlignmentFile(output_name+".trans.bam", template = bam_in, mode = "wb")
  bam_out_s = pysam.AlignmentFile(output_name+".bad.bam", template = bam_in, mode = "wb")
